...
- 📅 **时间信息** - 崩溃时间、系统运行时长
- 🐧 **内核版本** - 自动提取内核版本和发布信息
- 📊 **崩溃场景** - 智能分析崩溃发生的场景和上下文
- 📦 **模块识别** - 识别可疑模块和涉及的模块列表

### 🔗 调用栈深度分析 (NEW v1.2)
- 🧬 **函数调用关系** - 分析 caller-callee 关系和调用类型
- 🎯 **执行上下文** - 识别 syscall、irq、workqueue 等上下文
- 🛤️ **子系统追踪** - 追踪调用路径经过的内核子系统
- 💡 **场景推断** - 基于调用栈推断可能的崩溃场景

### 🧮 寄存器变化历史追踪 (NEW v1.2)
- 📈 **寄存器历史** - 追踪关键寄存器的变化历史
- 📍 **故障定位** - 精确推断故障源和根因函数
- 📋 **参数分析** - 分析函数入参，识别可疑参数传递
- 🔗 **寄存器链** - 追踪寄存器值从来源到崩溃点的传递链
...
│   ├── panic_overview.py # 🆕 Panic 概述提取器 (NEW v1.2)
│   ├── callstack_analyzer.py # 🆕 调用栈深度分析器 (NEW v1.2)
│   ├── register_analyzer.py  # 🆕 寄存器历史分析器 (NEW v1.2)
│   ├── log_parser.py     # kernel log 解析
│   └── context_builder.py # AI context 构建 (增强版)
├── tools/                # 工具网关
